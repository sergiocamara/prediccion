...
function valores = get_prediction_for_day_n(day)
    ...
    fid = fopen(strcat('IGH_SCS_',num2str(day + 15),'_abril_2020.csv'),'w'); % Create the CSV that will contain the predictions
    ...
    for i=1:length(name_ccaa)
        ...
        for j=1:length(variables)  % Iterate over the variables of each CCAA
            ...
            for w=1:7
                ccaa = iso_ccaa(i);
                valores{w, 1} = ccaa{1};
                next_day = datetime(output.historic{i}.label_x{index_of_day}, 'InputFormat', 'dd-MM-yyyy') + caldays(1:w);
                valores{w, 2} = datestr(next_day(w));
